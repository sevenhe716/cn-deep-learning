...
def create_lookup_tables(text):
    ...
    int_to_vocab = dict(enumerate(words_counter.keys()))
...
def get_batches(int_text, batch_size, seq_length):
    ...
    for i in range(0, n_batches):
        ...
        inputs_batch = int_text[start:end]
        target_batch = int_text[start+1:end+1]
        if len(target_batch) < batches_size:
            target_batch.append(int_text[0])
        
        batches[i][0] = np.array(inputs_batch).reshape([batch_size, seq_length])        
        batches[i][1] = np.array(target_batch).reshape([batch_size, seq_length])
